...
const GHE_VERSIONS = ["2.15", "2.16", "2.17"];
...
function writeRoutesFiles(version, routes) {
  ...
  writeFileSync(
    ...
    JSON.stringify(newRoutesSorted, null, 2) + "\n"
  );
  writeFileSync(
    `ghe-${version}/enterprise-admin.json`,
    JSON.stringify(newRoutesSorted.enterpriseAdmin, null, 2) + "\n"
  );
  writeFileSync(
    `ghe-${version}/index.js`,
  ...
  writeFileSync(
    `ghe-${version}/README.md`,
    `# @octokit/plugin-enterprise-rest/ghe-${version}
