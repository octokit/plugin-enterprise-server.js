...
octokit.activity.listIssueEventsForRepo(owner, page, per_page, repo)
...
octokit.codesOfConduct.getConductRepoCode(owner, repo)
...
octokit.git.getRefs(owner, page, per_page, repo)
octokit.git.getTag(owner, repo, tag_sha)
octokit.git.getTree(owner, repo, tree_sha)
octokit.git.getTreeRecursively(owner, repo, tree_sha)
...
octokit.issues.addLabels(number, owner, repo)
...
octokit.issues.replaceLabels(number, owner, repo)
...
octokit.projects.update(body, name, organization_permission, page, per_page, project_id, public, state)
...
octokit.repos.uploadReleaseAsset(headers, label, name, owner, release_id, repo)
