...
octokit.enterpriseAdmin.getReplicationStatus({ uuid, cluster_roles, origin });
octokit.enterpriseAdmin.getRepoStats();
octokit.enterpriseAdmin.getSettings({ origin });
octokit.enterpriseAdmin.getUserStats();
octokit.enterpriseAdmin.getVersion({ uuid, cluster_roles, origin });
...
octokit.actions.listWorkflowRunArtifacts({owner, repo, run_id, per_page, page, name});
...
octokit.actions.setCustomOidcSubClaimForRepo({owner, repo, use_default, include_claim_keys});
...
octokit.checks.create({owner, repo, name, head_sha, details_url, external_id, status, started_at, conclusion, completed_at, output, actions});
