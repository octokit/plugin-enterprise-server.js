...
octokit.enterpriseAdmin.createGlobalHooks({ name, config, events, active });
...
octokit.enterpriseAdmin.createIndexingJob({ target });
...
octokit.enterpriseAdmin.listPublicKeys({ per_page, page });
...
octokit.actions.deleteSecretFromRepo({ owner, repo, secret_name });
...
octokit.actions.downloadWorkflowJobLogs({ owner, repo, job_id });
...
octokit.actions.getSecret({ owner, repo, secret_name });
...
octokit.actions.listSecretsForRepo({ owner, repo, per_page, page });
...
octokit.activity.getThreadSubscription({ last_read_at });
...
octokit.activity.markNotificationsAsRead({ last_read_at });
...
octokit.codeScanning.listAlertsForRepo({ owner, repo, state, ref });
octokit.codeScanning.listRecentAnalyses({ owner, repo, ref, tool_name });
...
octokit.orgs.updateWebhook({ org, hook_id, config, events, active });
...
octokit.pulls.createReviewCommentAlternative({
  owner,
  repo,
  pull_number,
  body,
...
octokit.repos.downloadArchive({ owner, repo, archive_format, ref });
...
octokit.teams.addOrUpdateProjectPermissions({
  team_id,
  project_id,
  permission,
});
